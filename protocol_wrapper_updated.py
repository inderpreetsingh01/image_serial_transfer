...
class ProtocolWrapper(object):
    ...
    def __init__(self,
        ...
        self.last_message = [None]*self.num_packets               #*self.num_packets
        self.message_buf = ''
        self.last_error = ''

        

    def msg_to_packets(self, message):

        lst_wrapped = ''
        bits_per_pack = int(len(message)/self.num_packets)
        for i in range(self.num_packets):
            wrapped = self.wrap(message[bits_per_pack*i:bits_per_pack*(i+1)], i)
            lst_wrapped+=wrapped
        return lst_wrapped

    def wrap(self, message, frame_id):
        """ Wrap a message with header, footer and DLE according
            to the settings provided in the constructor.
        """	
        wrapped = self.header
        print(chr(frame_id))
        wrapped += chr(frame_id)
        for b in message:
    ...
    # internal state
    (WAIT_HEADER, IN_MSG, AFTER_DLE, IN_FRAME_ID) = range(4)
    ...
    def input(self, new_byte):
        ...
        elif self.state == self.IN_FRAME_ID:
            #print(new_byte)
            self.frame_id = ord(new_byte)
            print(self.frame_id)
            self.state = self.IN_MSG
            return ProtocolStatus.IN_FRAME_ID
    ...
    def _finish_msg(self):
        self.state = self.WAIT_HEADER
        self.last_message[self.frame_id] = self.message_buf
        self.message_buf = ''
